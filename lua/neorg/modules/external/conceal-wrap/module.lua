...
---Same as above, but using TS
module.private.visible_text_width_ts = function(buf, start, _end, target)
    -- offset x by inline virtual text
    local width = 0
    -- track positions that are concealed by extmarks
    local extmark_concealed = {}
    ...
    -- so we'll still need some type of loop over each char probably... or we can sort TS node
    -- ranges. I think that could be faster for large paragraphs

    -- I'm not sure that we need to query for things actually, we could just use this in the same
    -- way we're using the other thing currently, and just hope that it's significantly faster
    ...
    for r = start[1], _end[1] do
        ...
        for c = start_c, end_c do
            ...
            -- )
            -- Hmmm, this is probably the same information, right?
            local res = vim.treesitter.get_captures_at_pos(buf, r, c)
            -- P(res)
            for _, hl in ipairs(res) do
                if hl.capture == "conceal" and not extmark_concealed[c + 1] then
                    width = width - 1
